...
public class xlh {
    ...
    // 步骤二：调用ObjectOutputStream对象的writeObject输出可序列化对象。
        try {
             ObjectOutputStream outputBuffer=new ObjectOutputStream(new FileOutputStream("D:\\JavaCode\\study\\src\\data.txt"));
    ...
    //反序列化并不会调用构造方法。反序列的对象是由JVM自己生成的对象，不通过构造方法生成
        try (//创建一个ObjectInputStream输入流
             ObjectInputStream ois = new ObjectInputStream(new FileInputStream("D:\\JavaCode\\study\\src\\data.txt"))) {
