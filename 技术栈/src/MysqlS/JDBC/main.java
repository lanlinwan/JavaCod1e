...
import java.util.*;
//JDBC概念:
//●MysqlS.JDBC 就是使用Java语言操作关系型数据库的一套API
//全称: ( Java DataBase Connectivity ) Java数据库连接
//JDBC本质:
//官方(sun公司) 定义的- -套操作所有关系型数据库的规则，即接口
//各个数据库厂商去实现这套接口，提供数据库驱动jar包
//我们可以使用这套接口(MysqlS.JDBC) 编程，真正执行的代码是驱动jar包
//JDBC好处:
...
public class main {
    public static void main(String[] args) {
        ...
        try {
            //1.注册驱动
//            Driver driver=new com.MysqlS.mysql.jdbc.Driver();//多态:父类型引用指向子类对象(mysql驱动)
//            DriverManager.registerDriver(driver);
            //第二种注册驱动,更常用,因为参数是一个字符串,可以写到配置文件中
            Class.forName("com.mysql.jdbc.Driver");

            //2.获取连接
            conn = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/lan?useSSL=false&serverTimezone=UTC&serverTimezone=GMT%2B8",
                    "root",
                    "admin123");
