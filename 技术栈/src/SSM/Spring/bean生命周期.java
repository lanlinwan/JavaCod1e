...
public class bean生命周期 {
    //bean 是由 SSM.Spring IoC 容器实例化、组装和管理的对象。
    //它们是构成用户应用程序主干的对象。
    //Bean 是基于用户提供给容器的配置元数据创建。

    //bean scope:
    // Singleton - 每个 SSM.Spring IoC 容器仅有一个单实例。
    // Prototype - 每次请求都会产生一个新的实例。
    // Request - 每一次 javaweb.HTTP 请求都会产生一个新的实例，并且该 bean 仅在当前 javaweb.HTTP 请求内有效。
    ...
    //spring bean 中实现 Aware回调接口有什么用?
    //
    //在Spring Bean中实现Aware回调接口可以让Bean在初始化时访问Spring容器中的其他Bean。
    //具体来说，当一个Bean实现了一个Aware回调接口（如BeanNameAware、BeanFactoryAware、ApplicationContextAware等）时，
    //Spring容器会在初始化该Bean之前，自动回调实现的方法，从而让Bean获得容器中的其他Bean或容器相关的上下文信息。
    //
    //这些Aware回调接口的作用如下：
    //    BeanNameAware：通过实现该接口，可以让Bean在初始化时自动注入容器中该Bean的名称。
    //    BeanFactoryAware：通过实现该接口，可以让Bean在初始化时自动注入Spring容器本身的实例（即BeanFactory）。
    //                     可以直接使用这个BeanFactory来获取或创建其他的Bean。
    //    ApplicationContextAware：通过实现该接口，可以让Bean在初始化时自动注入Spring应用程序上下文的实例（即ApplicationContext）。
    //                             这个ApplicationContext实例提供了对当前Spring应用程序上下文的访问，包括其他Bean的定义、配置信息等。
    //
    //通过实现Aware回调接口，我们可以方便地访问容器中的其他Bean或上下文信息，并在初始化时进行相应的处理或准备工作。
    // 这对于一些需要依赖其他Bean或容器特定功能的Bean来说非常有用。
